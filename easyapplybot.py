...
class EasyApplyBot:
    ...
    def fill_out_fields(self):
        ...
        for field in fields:
            ...
            elif "Phone country code" in field.text:
                dropdown = field.find_element(By.TAG_NAME, "select")
                select = Select(dropdown)
                # Assuming country_code is the desired country code, you can replace it with the appropriate value
                country_code = "United States (+1)"
                select.select_by_visible_text(country_code)
            elif "Home address" in field.text:
                field_input = field.find_element(By.TAG_NAME, "input")
                field_input.clear()
                field_input.send_keys("New York, United States")
